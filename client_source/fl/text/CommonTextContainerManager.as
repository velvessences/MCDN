package fl.text
{
   ...
   internal class CommonTextContainerManager
   {
      ...
      {
         if(Boolean(this.textFlow) && Boolean(this.textFlow.formatResolver != null) && (param1 > 0 || param2 < length))
         {
            throw new Error("This method cannot be used on a text field with a style sheet.");
         }
         this.compose();
      ...
      public function getTextFormat(param1:int, param2:int) : TextFormat
      {
         ...
         if(param1 < 0)
         {
            _loc6_ = int(this.textFlow.getAbsoluteStart());
         ...
         {
            _loc13_ = Number(Number.NaN);
            _loc14_ = _loc8_.lineHeight;
            if(_loc14_)
            {
               if(_loc14_.indexOf("%") == -1)
               {
                  _loc13_ = Number(Number(_loc14_));
               }
               else
               {
                  _loc15_ = Number(this.textFlow.findLeaf(_loc6_).computedFormat.fontSize);
         ...
         else
         {
            ...
            {
               _loc17_ = int(_loc9_.tabStops.length);
               _loc18_ = 0;
