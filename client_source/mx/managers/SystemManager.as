package mx.managers
{
   ...
   public class SystemManager extends MovieClip implements IChildList, IFlexDisplayObject, IFlexModuleFactory, ISystemManager
   {
      ...
      mx_internal var preloader:mx.preloaders.Preloader;
      ...
      {
         return this.topLevelWindow ? this.topLevelWindow.getExplicitOrMeasuredHeight() : Number(loaderInfo.height);
      }
      
      public function get measuredWidth() : Number
      {
         return this.topLevelWindow ? this.topLevelWindow.getExplicitOrMeasuredWidth() : Number(loaderInfo.width);
      }
      ...
      mx_internal function initialize() : void
      {
         ...
         this.preloader = new mx.preloaders.Preloader();
      ...
      private function initializeTopLevelWindow(param1:Event) : void
      {
         ...
         {
            _loc2_ = Number(loaderInfo.width);
            _loc3_ = Number(loaderInfo.height);
         }
      ...
      private function Stage_resizeHandler(param1:Event = null) : void
      {
         ...
         try
         {
            m = Number(loaderInfo.width);
            n = Number(loaderInfo.height);
         }
         catch(error:Error)
         {
            if(!_screen)
         ...
         catch(error:SecurityError)
         {
            if(hasEventListener("getScreen"))
            {
               ...
               if(_screen)
               {
                  w = Number(_screen.width);
                  h = Number(_screen.height);
               }
            }
         }
         x = (m - w) / 2;
