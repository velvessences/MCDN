package starling.text
{
   ...
   public class TextField extends DisplayObjectContainer
   {
      ...
      private function renderText(param1:Number, param2:Rectangle) : BitmapData
      {
         ...
         var _loc10_:Number = Number(sNativeTextField.textWidth);
         var _loc3_:Number = Number(sNativeTextField.textHeight);
         if(isHorizontalAutoSize)
         {
            sNativeTextField.width = _loc5_ = Number(Math.ceil(_loc10_ + 5));
         }
         if(isVerticalAutoSize)
         {
            sNativeTextField.height = _loc11_ = Number(Math.ceil(_loc3_ + 4));
         }
         if(_loc5_ < 1)
         {
            _loc5_ = 1;
      ...
      private function calculateFilterOffset(param1:flash.text.TextField, param2:String, param3:String) : Point
      {
         ...
         {
            _loc9_ = Number(param1.textWidth);
            _loc4_ = Number(param1.textHeight);
            _loc11_ = new Rectangle();
            for each(var _loc18_ in _loc17_)
            {
               _loc5_ = Number("blurX" in _loc18_ ? _loc18_["blurX"] : 0);
               _loc6_ = Number("blurY" in _loc18_ ? _loc18_["blurY"] : 0);
               _loc14_ = Number("angle" in _loc18_ ? _loc18_["angle"] : 0);
               _loc13_ = Number("distance" in _loc18_ ? _loc18_["distance"] : 0);
      ...
      private function createComposedContents() : void
      {
         ...
         var _loc4_:Number = Number(mHitArea.width);
         var _loc1_:Number = Number(mHitArea.height);
      ...
      private function updateBorder() : void
      {
         ...
         var _loc3_:Number = Number(mHitArea.width);
         var _loc2_:Number = Number(mHitArea.height);
